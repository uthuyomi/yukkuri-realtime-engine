...
Step 10-C removed AqKanji2Koe assets from all local reachable refs, preserving local files. GitHub main still contains the old history and requires separately authorized replacement. Restricted objects also remain in the private backup and may remain unreachable in the local object database; neither is a publication source. See [audit report](docs/release-quality.md).

The project's [MIT License](LICENSE) covers original project code and project-authored documentation unless otherwise noted, not third-party dependencies or assets. It does not grant AQUEST usage or redistribution rights; obtain software and applicable rights separately from AQUEST. The separate historical-asset publication decision remains unresolved; see the [release report](docs/release-quality.md).
