...
Run `python scripts/release_check.py --ref HEAD` before `git archive`/tagging. Step 10-D committed the prepared work and replaced advertised GitHub main with the cleaned history using an owner-authorized explicit lease. A fresh GitHub clone verified the restricted assets are unreachable from main. This does not certify GitHub backend/cache erasure. Tags, releases and package publication still require separate owner authorization. See [release report](docs/release-quality.md).
...
Original project code and project-authored documentation are licensed under [MIT](LICENSE) unless otherwise noted. Third-party dependencies and assets retain their own licenses and terms. Obtain AquesTalk/AqKanji2Koe and applicable rights separately from AQUEST; MIT grants no rights to their binaries, dictionaries, SDK files or keys. Project license finalization is complete; restricted AQUEST assets are absent from local and advertised remote main history. GitHub retention and other clones are outside that claim; release/publication remains separately authorized.
